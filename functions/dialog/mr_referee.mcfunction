...
execute if entity @s[scores={Dialog=2..},tag=!done] as @e[tag=hostile,tag=!dead] unless entity @s[type=!minecraft:player,type=!minecraft:skeleton] if score @s Stage = @e[distance=..0.1,limit=1] Stage run tag @s add waiting_for_referee
