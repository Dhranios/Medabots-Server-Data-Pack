...
execute at @s as @e[x=-1772,y=50,z=-281,tag=kid,distance=..1] run scoreboard players operation @s DialogNr = #temp DialogNr
execute if entity @s[scores={Dialog=1..95}] at @e[tag=kid,tag=this_dialog,limit=1] facing entity @s feet run teleport @e[distance=..0.1,tag=kid,tag=this_dialog,limit=1] ~ ~ ~ ~ ~
...
execute if entity @s[scores={Dialog=97..}] as @e[tag=kid,tag=this_dialog,limit=1] at @s run teleport @s ^ ^ ^0.2125
