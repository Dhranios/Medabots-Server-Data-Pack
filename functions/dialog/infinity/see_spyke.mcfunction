...
advancement grant @s[scores={Dialog=1}] only medabots_server:wave_1/story_progression see_spyke
...
execute at @s as @e[x=-361,y=55,z=-83,tag=spyke,distance=..1] run scoreboard players operation @s DialogNr = @a[distance=..0.1,limit=1,scores={Dialog=1}] DialogNr
execute if entity @s[scores={Dialog=1..}] at @e[tag=spyke] facing entity @s feet if score @s DialogNr = @e[distance=..0.1,limit=1,sort=nearest,tag=spyke] DialogNr run teleport @e[distance=..0.1,limit=1,sort=nearest,tag=spyke] ~ ~ ~ ~ ~
