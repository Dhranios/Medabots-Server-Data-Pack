...
execute if entity @s[tag=in_ground,tag=ally_team] run scoreboard players set @e[distance=..3,scores={Fly=1..},tag=hostile,tag=!ally_medabot] LegsArmor 0
execute if entity @s[tag=in_ground,tag=enemy_team] run scoreboard players set @e[distance=..3,scores={Fly=1..},tag=hostile,tag=!enemy_medabot] LegsArmor 0