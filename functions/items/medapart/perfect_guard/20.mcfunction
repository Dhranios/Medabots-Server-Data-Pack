...
execute if entity @s[scores={ScoutTime=1..},tag=ally_medabot] run scoreboard players set @a[distance=..16,tag=ally_medabot] PerfectGuardTime 200
execute if entity @s[scores={ScoutTime=1..},tag=enemy_medabot] run scoreboard players set @e[distance=..16,tag=enemy_medabot] PerfectGuardTime 200

# Remove from head uses if this was the head
scoreboard players remove @s[scores={PerfectGuard=2}] HeadUses 1