...
execute if entity @s[scores={DeathTime=2},type=minecraft:player] run function medabots_server:other/lose_medallars
execute if entity @s[scores={DeathTime=2},type=minecraft:player,tag=!practice_battle] if entity @a[scores={Killer=1..},tag=!practice_battle] run function medabots_server:medaparts/drop
execute if entity @s[scores={DeathTime=2},type=minecraft:player,tag=!practice_battle] if entity @e[tag=enemy_medabot,nbt=!{DeathLootTable:"medabots_server:entities/battle_enemies/practice_round"}] run function medabots_server:medaparts/drop
execute if entity @s[scores={DeathTime=2},type=!minecraft:player] run loot spawn ~ ~3 ~ kill @s
execute if entity @s[scores={DeathTime=2}] positioned ~ ~3 ~ run tag @e[distance=..1,type=minecraft:item] add medabot_loot
