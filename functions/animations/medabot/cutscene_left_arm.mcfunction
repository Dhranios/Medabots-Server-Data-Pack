...
# Equipped arms
execute if entity @e[distance=..0.1,tag=cutscene,tag=!medabot_model,tag=left_arm_selected] run tag @s add selected
execute if entity @s[tag=selected] unless entity @e[distance=..0.1,tag=hostile,tag=cutscene,tag=left_arm_selected] run tag @s remove selected

# Walk animation
execute unless entity @s[nbt={Pose:{Head:[0.001f]}}] run data merge entity @s {Pose:{Head:[0.0f,0.0f,0.001f]}}
execute if entity @e[distance=..0.1,tag=cutscene,tag=!medabot_model,type=!minecraft:player,tag=walking] run tag @s add walking
