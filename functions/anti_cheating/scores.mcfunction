...
# Deactivate parts
execute unless entity @s[scores={FlyCourse=0..}] run scoreboard players reset @s[tag=!stage_builder,tag=!hostile,tag=now,scores={Stage=0..}] Stage
execute unless entity @s[scores={FlyCourse=0..}] run tag @s[tag=!stage_builder,tag=!hostile,tag=now,scores={Stage=0..}] remove now
execute unless entity @s[scores={FlyCourse=0..}] run tag @s[tag=!stage_builder,tag=!hostile,tag=!now,scores={Stage=0..}] add now
