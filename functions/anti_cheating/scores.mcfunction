...
# Set scores
scoreboard players set @s[scores={Error=1..,Battle=1..3}] Battle 0
scoreboard players set @s[scores={Death=1..,Battle=1..3}] Battle 0
execute if entity @s[scores={Battle=1..3},tag=!hostile,tag=add_hostile] at @s run function medabots_server:gamemodes/default/set_stats
tag @s[scores={Battle=1..3},tag=!hostile,tag=!add_hostile] add add_hostile
