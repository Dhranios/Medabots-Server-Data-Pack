...
execute if entity @s[scores={Death=1..},type=minecraft:player] run tellraw @a {"translate":"medabots_server:death.melee_trap","with":[{"selector":"@s"},{"selector":"@e[distance=..0.1,type=minecraft:area_effect_cloud,tag=melee_trap]"}]}
tag @s[scores={Death=1..}] add had_death
playsound medabots_server:entity.medabot.attack.trap_hit player @a ~ ~ ~ 1
kill @e[distance=..0.1,type=minecraft:area_effect_cloud,tag=melee_trap]