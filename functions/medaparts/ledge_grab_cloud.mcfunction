...
execute if entity @s[nbt={Age:0}] positioned ~ ~1 ~ at @a[distance=..1] run playsound medabots_server:entity.medabot.move.ledge_grab player @a ~ ~ ~ 1

# Make fall
tag @s[tag=dead] add drop
execute if entity @s[tag=drop] positioned ~ ~1 ~ at @a[distance=..1] run playsound medabots_server:entity.medabot.move.ledge_grab_drop player @a ~ ~ ~ 1
