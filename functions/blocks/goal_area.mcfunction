...
execute as @e[tag=rubberobo,type=!minecraft:player] unless score @s Stage = #temp Stage run tag @e[distance=..0.7,tag=goal_area] add stop
execute as @e[tag=jammy] unless score @s Stage = #temp Stage run tag @e[distance=..0.7,tag=goal_area] add stop
