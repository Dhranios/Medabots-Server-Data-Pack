...
execute if entity @s[scores={Dialog=1..191}] run scoreboard players set @e[tag=waiting_for_referee,scores={Battle=1}] Battle 2
...
execute if entity @s[scores={Dialog=2}] as @e[tag=waiting_for_referee] if score @s Stage = @e[distance=..0.1,limit=1] Stage run tag @s remove waiting_for_referee
...
execute if entity @s[scores={Dialog=192}] as @e[tag=hostile] unless entity @s[type=!minecraft:player,type=!minecraft:skeleton] if score @s Stage = @e[distance=..0.1,limit=1] Stage run scoreboard players set @s[scores={Battle=2}] Battle 1
data merge entity @s[scores={Dialog=193}] {Small:1b,Invisible:1b,ArmorItems:[{},{},{},{}]}
execute if entity @s[scores={Dialog=193}] run stopsound @a[distance=..50] music medabots_server:music.entity.mr_referee
