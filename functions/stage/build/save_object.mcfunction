data modify entity @e[tag=build_stage,limit=1] Item.tag.medabots_server.stage_data append value {position:[0,0,0],object:[""],additional_data:[0,0,0,0,0,0,0,0]}
...
execute if entity @s[scores={HomeX=0..}] store result entity @e[tag=build_stage,limit=1] Item.tag.medabots_server.stage_data[-1].additional_data[0] int 1 run scoreboard players get @s HomeX
...
execute if entity @s[scores={HomeZ=0..}] store result entity @e[tag=build_stage,limit=1] Item.tag.medabots_server.stage_data[-1].additional_data[2] int 1 run scoreboard players get @s HomeZ
