...
execute unless entity @s[tag=racer] if score @s Money >= @a[x=-89,y=120,z=-82,dx=6,dy=3,dz=8,tag=0,limit=1] FlyCourseFee run data merge block -89 121 -79 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"execute if entity @s[tag=!racer,tag=!0] run function medabots_server:stage/join/medium_fly_course_1/7"}}'}
...
execute unless entity @s[tag=racer] if score @s Money >= @a[x=-89,y=120,z=-82,dx=6,dy=3,dz=8,tag=0,limit=1] FlyCourseFee run tag @s add 6
