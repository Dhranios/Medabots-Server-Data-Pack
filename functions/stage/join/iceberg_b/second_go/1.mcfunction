...
execute if entity @s[scores={Dialog=0,Error=0}] run data merge block -1794 51 -437 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"execute if entity @s[tag=!enemy_medabot,tag=!-1] run function medabots_server:stage/join/iceberg_b/second_go/2"}}'}
