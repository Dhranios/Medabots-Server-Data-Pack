...
execute if entity @s[scores={Dialog=0,Error=0}] run data merge block -1948 52 -640 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function medabots_server:stage/sign/generic/no_pvp_on"}}'}
...
scoreboard players set @s[scores={Dialog=0,Error=0}] StageVersion 2
tag @s[scores={Dialog=0,Error=0}] add 0
execute if entity @s[scores={Dialog=0,Error=0}] run function medabots_server:stage/create/woods_battle/action_floors_1
