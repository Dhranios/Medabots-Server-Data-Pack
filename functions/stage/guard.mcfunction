...
execute unless entity @s[scores={Time=-30..}] run function medabots_server:stage/guard_scores

# Attack
scoreboard players remove @s[scores={Time=1..}] Time 1
execute if entity @s[scores={Time=0}] positioned ^ ^ ^1 if entity @a[tag=hostile,distance=..1.05,tag=!enemy_medabot] run function medabots_server:stage/guard_attack

# Walk to target or random location
execute if entity @s[scores={Time=0}] run function medabots_server:stage/guard_wander
