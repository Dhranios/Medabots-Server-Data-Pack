...
execute if entity @s[tag=vs_cpus] positioned -1999 45 -593 rotated -180 0 run function medabots_server:stage/create/icefield_battle/random_cpu
...
execute if entity @s[tag=!vs_cpus] run data merge block -1998 52 -632 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"execute if entity @s[tag=!medabot,tag=!-1] run function medabots_server:stage/join/icefield_battle/locked_out/0"}}'}
