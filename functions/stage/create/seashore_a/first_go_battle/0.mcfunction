...
execute unless entity @a[x=-1570,y=51,z=-247,dx=63,dy=4,dz=63,tag=0] run function medabots_server:stage/create/seashore_a/first_go_battle/0_cpu
...
execute if entity @e[type=minecraft:area_effect_cloud,tag=referee_marker,tag=1,tag=selected] run summon minecraft:armor_stand -1535 45 -227 {CustomName:'{"translate":"medabots_server:entity.mr_referee"}',Tags:["mr_referee"],Invisible:1b,Rotation:[90.0f,0.0f]}
execute if entity @e[type=minecraft:area_effect_cloud,tag=referee_marker,tag=2,tag=selected] run summon minecraft:armor_stand -1540 45 -228 {CustomName:'{"translate":"medabots_server:entity.mr_referee"}',Tags:["mr_referee"],Invisible:1b,Rotation:[-90.0f,0.0f]}
kill @e[type=minecraft:area_effect_cloud,tag=referee_marker]
bossbar set medabots_server:seashore_a/robattle value 3600
bossbar set medabots_server:seashore_a/robattle players @a[scores={Stage=11}]