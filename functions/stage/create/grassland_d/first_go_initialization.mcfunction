...
execute if entity @s[tag=!vs_cpus] run data merge block -1569 52 -750 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"execute if entity @s[tag=!medabot] run function medabots_server:stage/join/grassland_d/first_go/0"}}'}
...
execute if entity @s[tag=!vs_cpus] run data merge block -1571 52 -750 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function medabots_server:stage/join/grassland_d/first_go/cancel"}}',Text3:'{"translate":"medabots_server:sign.stage.cancel"}'}
