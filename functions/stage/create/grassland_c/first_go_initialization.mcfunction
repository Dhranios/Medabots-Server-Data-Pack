...
execute if entity @s[tag=!vs_cpus] run data merge block -1538 51 -480 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"execute if entity @s[tag=!medabot,tag=!-1] run function medabots_server:stage/join/grassland_c/first_go/0"}}'}
...
execute if entity @s[tag=!vs_cpus] run data merge block -1540 51 -480 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function medabots_server:stage/join/grassland_c/first_go/cancel"}}',Text3:'{"translate":"medabots_server:sign.stage.cancel"}'}
