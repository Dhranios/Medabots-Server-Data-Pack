...
execute if entity @s[tag=vs_cpus] positioned -1264 41 5 rotated 0 0 run function medabots_server:stage/create/lagdou_ruins_8/random_cpu
...
execute if entity @s[tag=!vs_cpus] run data merge block -1289 50 1 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"execute if entity @s[tag=!medabot,tag=!-1] run function medabots_server:stage/join/lagdou_ruins_8/darkness/0"}}'}
