...
execute unless entity @a[x=-1666,y=51,z=-477,dx=94,dy=4,dz=92,tag=2] positioned -1650 45 -466 run function medabots_server:spawn_entities/enemy_medabot/baroncastle/dondoguu/fly_falcon/baroncastle/monkey
...
execute if entity @e[type=minecraft:area_effect_cloud,tag=referee_marker,tag=1,tag=selected] run summon minecraft:armor_stand -1648 45 -464 {CustomName:'{"translate":"medabots_server:entity.mr_referee"}',Tags:["mr_referee"],Invisible:1b,Rotation:[90.0f,0.0f]}
execute if entity @e[type=minecraft:area_effect_cloud,tag=referee_marker,tag=2,tag=selected] run summon minecraft:armor_stand -1651 45 -464 {CustomName:'{"translate":"medabots_server:entity.mr_referee"}',Tags:["mr_referee"],Invisible:1b,Rotation:[-90.0f,0.0f]}
