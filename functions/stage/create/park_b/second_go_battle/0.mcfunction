...
execute unless entity @a[x=-1857,y=51,z=-730,dx=94,dy=4,dz=94,tag=0] positioned -1810 45 -653 run function medabots_server:spawn_entities/enemy_medabot/sailor_multi/pure_mermaid/sailor_multi/sailor_multi/spider
...
execute if entity @e[type=minecraft:area_effect_cloud,tag=referee_marker,tag=1,tag=selected] run summon minecraft:armor_stand -1804 45 -657 {CustomName:'{"translate":"medabots_server:entity.mr_referee"}',Tags:["mr_referee"],Invisible:1b,Rotation:[90.0f,0.0f]}
execute if entity @e[type=minecraft:area_effect_cloud,tag=referee_marker,tag=2,tag=selected] run summon minecraft:armor_stand -1817 45 -658 {CustomName:'{"translate":"medabots_server:entity.mr_referee"}',Tags:["mr_referee"],Invisible:1b,Rotation:[90.0f,0.0f]}
