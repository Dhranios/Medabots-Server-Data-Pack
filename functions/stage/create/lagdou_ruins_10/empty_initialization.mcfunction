...
execute if entity @s[tag=vs_cpus] positioned -1361 41 -104 rotated 0 0 run function medabots_server:stage/create/lagdou_ruins_10/random_cpu
...
execute if entity @s[tag=!vs_cpus] run data merge block -1360 52 -114 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"execute if entity @s[tag=!medabot] run function medabots_server:stage/join/lagdou_ruins_10/empty/0"}}'}
execute if entity @s[tag=!vs_cpus] run tellraw @a {"translate":"medabots_server:message.stage.players.1","color":"green","with":[{"translate":"medabots_server:location.lagdou_ruins_10"}]}
