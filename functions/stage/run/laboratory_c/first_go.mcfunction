...
execute if entity @e[scores={Stage=28},tag=hide_normal_time] if score #temp Time matches 0 as @e[x=-1596,y=42,z=-620,dx=22,dy=7,dz=44,tag=mr_referee] at @s run function medabots_server:stage/referee_decides
scoreboard players reset #temp Time
execute if entity @s[scores={Battle=0}] run function medabots_server:stage/clean_up/laboratory_c/first_go
execute if entity @s[scores={Battle=0}] run function medabots_server:other/death
execute if block -1586 44 -598 minecraft:iron_door[open=true] if entity @s[x=-1587,y=44,z=-601,dx=3,dy=3,dz=3] run function medabots_server:stage/create/laboratory_c/first_go_battle/0
...
execute if entity @s[scores={BattlingMedabots=1,Battle=1..}] run function medabots_server:stage/clean_up/laboratory_c/first_go
advancement grant @s[scores={BattlingMedabots=1,Battle=1..}] only medabots_server:stages/wave_1/laboratory_c_first_go
execute if entity @s[scores={BattlingMedabots=1,Battle=1..}] run bossbar set medabots_server:laboratory_c/robattle players
tag @s[scores={BattlingMedabots=1,Battle=1..},advancements={medabots_server:story_progression/infinity={doctor_harus_research=true,doctor_haru_defeated=false}}] add dialog_infinity_doctor_haru_defeated
