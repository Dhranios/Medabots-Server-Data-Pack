...
execute if entity @e[scores={Stage=28},tag=hide_normal_time] if score #temp Time matches 0 as @e[x=-1596,y=42,z=-620,dx=22,dy=7,dz=44,tag=mission,type=minecraft:area_effect_cloud] at @s run function medabots_server:stage/mission_time_up
...
execute if entity @s[scores={Battle=0}] run function medabots_server:stage/clean_up/laboratory_c/custom
...
execute as @e[x=-1596,y=42,z=-620,dx=22,dy=7,dz=44,tag=build_stage,limit=1] run data merge entity @s {Item:{tag:{display:{Name:'{"italic":false,"color":"white","translate":"medabots_server:item.custom_stage","with":[{"translate":"medabots_server:location.laboratory_c"}]}'}}}}
...
execute if entity @s[tag=clear_stage] positioned -1586 50 -599 run function medabots_server:stage/clear