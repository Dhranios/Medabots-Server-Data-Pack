...
execute if entity @e[scores={Stage=35},tag=hide_normal_time] if score #temp Time matches 0 as @e[x=-1856,y=41,z=-811,dx=61,dy=7,dz=78,tag=mr_referee] at @s run function medabots_server:stage/referee_decides
execute if entity @s[scores={Battle=0}] run function medabots_server:stage/clean_up/laboratory_d/custom
