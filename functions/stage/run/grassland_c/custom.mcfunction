...
execute if entity @e[scores={Stage=23},tag=hide_normal_time] if score #temp Time matches 0 as @e[x=-1570,y=41,z=-480,dx=65,dy=7,dz=63,tag=mr_referee] at @s run function medabots_server:stage/referee_decides
...
execute as @e[x=-1570,y=42,z=-480,dx=65,dy=7,dz=63,tag=build_stage,limit=1] run data merge entity @s {Item:{tag:{display:{Name:'{"italic":false,"color":"white","translate":"medabots_server:item.custom_stage","with":[{"translate":"medabots_server:location.grassland_c"}]}'}}}}
