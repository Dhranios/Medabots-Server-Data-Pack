...
execute if entity @e[x=-1754.5,y=44,z=-360.5,distance=..0.7,tag=mission,type=minecraft:area_effect_cloud,scores={Dialog=81}] unless entity @e[scores={Stage=13},tag=cannon,type=minecraft:zombie,tag=mission_entity] run tag @s add mission_success
...
execute if entity @s[tag=mission_success] run tag @e[scores={Stage=13,Medabot=0..,Battle=1..},type=minecraft:skeleton] add dead
execute if entity @s[tag=mission_success] as @a[scores={Stage=13,Medabot=0..,Battle=1..2},tag=enemy_medabot] run function medabots_server:stage/leave_server_enforced
