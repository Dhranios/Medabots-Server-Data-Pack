...
execute if block -1563 44 -235 minecraft:lime_wool run scoreboard players set @e[x=-1514.5,y=45,z=-229.5,distance=..0.7,tag=door,type=minecraft:area_effect_cloud,scores={PowerAmount=0}] PowerAmount 1
execute as @e[x=-1570,y=43,z=-247,dx=63,dy=7,dz=63] unless entity @s[scores={Stage=4}] run scoreboard players set @s Stage 4
execute if entity @s[tag=clear_stage] run function medabots_server:stage/clean_up/seashore_a/rubberobo
