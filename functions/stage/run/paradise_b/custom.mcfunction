...
execute as @e[x=-1665,y=42,z=-637,dx=64,dy=7,dz=63,tag=build_stage,limit=1] run data merge entity @s {Item:{tag:{display:{Name:'{"italic":false,"color":"white","translate":"medabots_server:item.custom_stage","with":[{"translate":"medabots_server:location.paradise","with":[{"text":"B"}]}]}'}}}}
...
execute if entity @s[tag=clear_stage] positioned -1634 51 -606 run function medabots_server:stage/clear