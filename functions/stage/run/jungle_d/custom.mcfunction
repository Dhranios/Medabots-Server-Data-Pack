...
execute as @e[x=-1665,y=42,z=-767,dx=61,dy=7,dz=61,tag=build_stage,limit=1] run data merge entity @s {Item:{tag:{display:{Name:'{"italic":false,"color":"white","translate":"medabots_server:item.custom_stage","with":[{"translate":"medabots_server:location.jungle","with":[{"text":"D"}]}]}'}}}}
...
execute if entity @s[tag=clear_stage] positioned -1635 51 -737 run function medabots_server:stage/clear