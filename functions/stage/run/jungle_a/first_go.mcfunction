...
execute if entity @e[x=-1651.5,y=45,z=-126.5,distance=..0.7,tag=mission,scores={Dialog=81}] run title @s[scores={BattlingMedabots=1}] title {"translate":"medabots_server:message.stage.mission.complete","color":"green"}
execute if entity @e[x=-1651.5,y=45,z=-126.5,distance=..0.7,tag=mission,scores={Dialog=81}] if block -1662 46 -124 minecraft:iron_door[open=false] as @s[scores={BattlingMedabots=1}] run setblock -1662 45 -124 minecraft:structure_block[mode=load]{rotation:"CLOCKWISE_90",name:"medabots_server:stage/open_door",mode:"LOAD"}
