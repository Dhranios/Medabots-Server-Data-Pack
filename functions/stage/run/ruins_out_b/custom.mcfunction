...
execute as @e[x=-1858,y=0,z=-536,dx=94,dy=50,dz=95,tag=build_stage,limit=1] run data merge entity @s {Item:{tag:{display:{Name:'{"italic":false,"color":"white","translate":"medabots_server:item.custom_stage","with":[{"translate":"medabots_server:location.ruins_out","with":[{"text":"B"}]}]}'}}}}
...
execute if entity @s[tag=clear_stage] positioned -1812 51 -489 run function medabots_server:stage/clear