execute store result score @s BattlingMedabots if entity @e[scores={Stage=100,Battle=1..,Medabot=0..}]
...
execute as @e[x=-2017,y=43,z=-581,dx=42,dy=7,dz=42,type=!minecraft:player] unless entity @s[scores={Stage=100}] run scoreboard players set @s Stage 100
execute if entity @s[scores={BattlingMedabots=1}] run function medabots_server:stage/clean_up/ruins_battle
