...
execute if entity @e[scores={Stage=57},tag=hide_normal_time] if score #temp Time matches 0 as @e[x=-1997,y=41,z=-424,dx=88,dy=7,dz=91,tag=mr_referee] at @s run function medabots_server:stage/referee_decides
execute if entity @s[scores={Battle=0}] run function medabots_server:stage/clean_up/iceberg_e/custom
...
execute as @e[x=-1997,y=41,z=-424,dx=88,dy=7,dz=91,tag=build_stage,limit=1] run data merge entity @s {Item:{tag:{display:{Name:'{"italic":false,"color":"white","translate":"medabots_server:item.custom_stage","with":[{"translate":"medabots_server:location.iceberg_e"}]}'}}}}
