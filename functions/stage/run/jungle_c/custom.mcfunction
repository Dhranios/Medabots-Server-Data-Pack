...
execute if entity @e[scores={Stage=24},tag=hide_normal_time] if score #temp Time matches 0 as @e[x=-1570,y=41,z=-545,dx=62,dy=7,dz=62,tag=mr_referee] at @s run function medabots_server:stage/referee_decides
execute if entity @s[scores={Battle=0}] run function medabots_server:stage/clean_up/jungle_c/custom
...
execute as @e[x=-1570,y=42,z=-545,dx=62,dy=7,dz=62,tag=build_stage,limit=1] run data merge entity @s {Item:{tag:{display:{Name:'{"italic":false,"color":"white","translate":"medabots_server:item.custom_stage","with":[{"translate":"medabots_server:location.jungle_c"}]}'}}}}
