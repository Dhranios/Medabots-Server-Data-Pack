...
execute as @e[x=-1665,y=0,z=-703,dx=62,dy=49,dz=63,tag=build_stage,limit=1] run data merge entity @s {Item:{tag:{display:{Name:'{"italic":false,"color":"white","translate":"medabots_server:item.custom_stage","with":[{"translate":"medabots_server:location.ruins_out","with":[{"text":"C"}]}]}'}}}}
...
execute if entity @s[tag=clear_stage] positioned -1635 50 -672 run function medabots_server:stage/clear