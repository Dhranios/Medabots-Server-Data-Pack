execute if entity @e[x=-1376,y=39,z=-276,dx=58,dy=10,dz=68,tag=mr_referee,tag=finished] store result score @s BattlingMedabots if entity @e[scores={Stage=40,Medabot=0..,Battle=1..}]
execute unless entity @e[scores={Stage=40,Medabot=0..,Battle=1..},tag=main] run tag @s add main
execute if entity @s[scores={BattlingMedabots=1,Battle=1..}] run function medabots_server:stage/clean_up/lagdou_ruins_5/empty
