...
tag @s add me
execute if entity @s[tag=medabot] run tellraw @a[tag=this_robattle] {"translate":"medabots_server:death.shooting_trap","with":[{"selector":"@s"},{"selector":"@e[distance=..3,type=minecraft:area_effect_cloud,tag=shooting_trap]"}]}
tag @s add had_death
tag @s add dying
