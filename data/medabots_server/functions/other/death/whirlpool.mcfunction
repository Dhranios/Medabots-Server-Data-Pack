...
tag @s add me
execute if entity @s[tag=medabot] run tellraw @a[tag=this_robattle] {"translate":"medabots_server:death.whirlpool","with":[{"selector":"@s"}]}
tag @s add had_death
tag @s add drowned
tag @s add dying
tag @s[tag=guard] add dead
tag @s[tag=bomb] add dead
