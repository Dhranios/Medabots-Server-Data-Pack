...
tag @s add me
execute if entity @s[tag=medabot] run tellraw @a[tag=this_robattle] {"translate":"medabots_server:death.crashed_on","with":[{"selector":"@s"},{"selector":"@e[distance=..0.1,tag=crasher,limit=1]"}]}
tag @s add dying
tag @s add had_death
