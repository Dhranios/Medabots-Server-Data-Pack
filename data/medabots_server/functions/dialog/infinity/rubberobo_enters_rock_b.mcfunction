...
execute if entity @s[scores={Dialog=1}] positioned 29 59 31 run function medabots_server:spawn_entities/cutscene/rubberobo
...
execute at @s as @e[x=29,y=59,z=33,tag=kid,distance=..1] run scoreboard players operation @s DialogNr = #temp DialogNr
...
execute if entity @s[scores={Dialog=400}] as @e[tag=rubberobo,tag=this_dialog,limit=1] at @s run teleport @s ~ ~ ~ facing 31 59 0
