execute as @e[x=0,y=50,z=0,dx=60,dy=7,dz=60,type=!minecraft:player] unless entity @s[scores={Stage=60}] run scoreboard players set @s Stage 60
...
teleport @a[scores={Stage=60},tag=medabot,distance=0.1..] 29 59 30 -180 0
...
teleport @s[scores={Stage=60,State=0}] 29 59 30 -180 0
tag @e[scores={Stage=60},type=!minecraft:player] add dead
scoreboard players reset @a[scores={Stage=60},distance=0.1..] StageVersion
scoreboard players reset @a[scores={Stage=60},distance=0.1..] Stage
data merge block 28 60 0 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function medabots_server:stage/sign/rock_f/play"}}'}
