...
execute if entity @s[scores={Dialog=2}] as @e[tag=cannon,type=minecraft:zombie,tag=this_mission] run tag @s remove enabled
execute if entity @s[scores={Dialog=2}] as @e[tag=!medabot,tag=this_mission] run tag @s add disabled
...
# Custom stage
execute if entity @s[scores={Dialog=81},tag=custom_stage] store result score @s BattlingMedabots if entity @e[scores={Medabot=0..,State=1..2},tag=this_mission]
execute if entity @s[scores={Dialog=81},tag=custom_stage,tag=!open_the_exit_door] run tag @e[distance=..0.7,tag=powered,tag=door,type=minecraft:area_effect_cloud] add no_open
execute if entity @s[scores={Dialog=81},tag=custom_stage,tag=open_the_exit_door] if entity @e[distance=..0.7,tag=powered,tag=door,type=minecraft:area_effect_cloud] run tag @s add mission_success
...
execute if entity @s[scores={Dialog=81},tag=custom_stage,tag=destroy_all_cannons] unless entity @e[tag=mission_entity,tag=this_mission,tag=cannon,type=minecraft:zombie] run tag @s add mission_success
...
execute if entity @s[tag=mission_success] run scoreboard players set @e[tag=door,tag=this_mission,distance=..0.7,type=minecraft:area_effect_cloud,scores={PowerNeeded=1..}] PowerNeeded 0
...
execute if entity @s[tag=mission_success] run tag @e[tag=door,tag=this_mission,tag=no_open,type=minecraft:area_effect_cloud] remove no_open
