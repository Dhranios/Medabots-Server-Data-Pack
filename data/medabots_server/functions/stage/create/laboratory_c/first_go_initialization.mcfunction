...
execute if entity @s[tag=!vs_cpus] run data merge block -1585 51 -620 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"execute if entity @s[tag=!medabot] run function medabots_server:stage/join/laboratory_c/first_go/0"}}'}
...
execute if entity @s[tag=!vs_cpus] run data merge block -1587 51 -619 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function medabots_server:stage/join/laboratory_c/first_go/cancel"}}',Text3:'{"translate":"medabots_server:sign.stage.cancel"}'}
