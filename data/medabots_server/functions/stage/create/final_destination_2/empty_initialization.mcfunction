...
execute if entity @s[tag=!vs_cpus] run data merge block 50 44 35 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"execute if entity @s[tag=!medabot] run function medabots_server:stage/join/final_destination_2/try"}}'}
execute if entity @s[tag=!vs_cpus] run setblock 52 45 33 minecraft:stone_button[face=wall,facing=east,powered=true]
execute if entity @s[tag=!vs_cpus] run data merge block 50 44 33 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function medabots_server:stage/join/final_destination_2/cancel"}}',Text3:'{"translate":"medabots_server:sign.stage.cancel"}'}