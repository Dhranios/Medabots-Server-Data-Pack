...
execute if entity @s[tag=vs_cpus,scores={ArenaType=0..1}] run function medabots_server:stage/create/lagdou_ruins_1/empty
...
execute if entity @s[tag=!vs_cpus] run data merge block -1297 52 -115 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"execute if entity @s[tag=!medabot] run function medabots_server:stage/join/lagdou_ruins_1/try"}}'}
execute if entity @s[tag=!vs_cpus] run setblock -1299 53 -117 minecraft:redstone_block
execute if entity @s[tag=!vs_cpus] run data merge block -1299 52 -115 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function medabots_server:stage/join/lagdou_ruins_1/cancel"}}',Text3:'{"translate":"medabots_server:sign.stage.cancel"}'}