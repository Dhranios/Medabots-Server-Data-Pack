...
execute if entity @s[tag=!cancel] if score #temp Money matches 1 run tag @s add no_saving
...
execute if entity @s[tag=!cancel] as @e[scores={Stage=1..},type=!minecraft:player] if score #temp Stage = @s Stage run tag @s add dead

execute if entity @s[tag=!cancel] store result score @s NeededPlayers run data get entity @e[tag=build_stage,limit=1] Item.tag.medabots_server.vs
