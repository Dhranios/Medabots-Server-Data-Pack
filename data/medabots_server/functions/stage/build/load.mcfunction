...
tellraw @s[tag=cancel,nbt={Inventory:[{tag:{medabots_server:{id:"medabots_server:custom_stage"}}}]}] {"translate":"medabots_server:message.stage.stage_builder.loading_wrong_stage","color":"red"}
tellraw @s[tag=cancel,nbt=!{Inventory:[{tag:{medabots_server:{id:"medabots_server:custom_stage"}}}]}] {"translate":"medabots_server:message.stage.stage_builder.no_stage_to_load","color":"red"}

execute as @e[scores={Stage=1..},type=!minecraft:player] if score #temp Stage = @s Stage run tag @s add dead

execute unless entity @s[tag=cancel] as @e[tag=build_stage] at @s run function medabots_server:stage/build/load_object
