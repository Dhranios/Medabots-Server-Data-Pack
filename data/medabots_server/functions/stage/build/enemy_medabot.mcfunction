...
execute if entity @s[tag=!failed] run data modify entity @e[distance=..0.7,tag=enemy_medabot,limit=1] ArmorItems[3].tag.medabots_server merge value {cpu_data:{team:"enemy",level:0,practice_battle:1b}}
execute if entity @s[tag=!failed] store result entity @e[distance=..0.7,tag=enemy_medabot,limit=1] ArmorItems[3].tag.medabots_server.cpu_data.level int 1 run scoreboard players get @s Level
