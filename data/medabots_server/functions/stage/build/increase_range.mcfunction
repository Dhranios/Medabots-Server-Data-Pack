tag @s add no
execute if entity @e[distance=..0.7,scores={Range=..22},tag=press_wall] run tag @s remove no
...
execute if entity @s[tag=!no] run scoreboard players add @e[distance=..0.7,scores={Range=0..}] Range 1
