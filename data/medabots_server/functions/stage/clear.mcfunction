...
execute store result score #temp Stage run data get entity @s[scores={Stage=1..}] Inventory[{Slot:-106b}].tag.medabots_server.stage
execute if score @s[nbt={Inventory:[{Slot:-106b,tag:{medabots_server:{id:"medabots_server:custom_stage"}}}]}] Stage = #temp Stage unless entity @s[scores={StageVersion=1..}] run tag @s add cleared_custom_stage
...
data modify entity @s[tag=cleared_custom_stage] Inventory[{Slot:-106b}].tag.display.Lore[2] set value '{"italic":false,"color":"white","translate":"medabots_server:item.custom_stage.cleared","with":[{"color":"green","translate":"medabots_server:item.custom_stage.cleared.yes"}]}'
