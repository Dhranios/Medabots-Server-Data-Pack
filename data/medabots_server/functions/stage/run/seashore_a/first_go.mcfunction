...
execute if entity @s[scores={Battle=0}] run function medabots_server:stage/clean_up/seashore_a
...
execute if entity @e[x=-1570,y=42,z=-247,dx=63,dy=7,dz=63,tag=mr_referee,type=minecraft:armor_stand,tag=finished] store result score @s BattlingMedabots if entity @e[scores={Stage=4,Medabot=0..,Battle=1..2}]
...
execute if entity @s[scores={BattlingMedabots=1,Battle=1..}] run function medabots_server:stage/clean_up/seashore_a
