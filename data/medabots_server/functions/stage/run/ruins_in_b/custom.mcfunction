...
execute if entity @e[scores={Stage=17},tag=hide_normal_time] if score #temp Time matches 0 as @e[x=0,y=9,z=0,dx=92,dy=49,dz=92,tag=mission,type=minecraft:area_effect_cloud] at @s run function medabots_server:stage/mission_time_up
...
execute if entity @s[scores={State=0},tag=!stage_builder] run function medabots_server:stage/clean_up/ruins_in_b
...
execute if entity @s[tag=add_bossbar] run bossbar set medabots_server:ruins_in_b/time players
execute if entity @s[tag=add_bossbar] run bossbar set medabots_server:ruins_in_b/robattle players @a[scores={Stage=17}]
execute if entity @s[tag=add_bossbar] store result bossbar medabots_server:ruins_in_b/robattle max run scoreboard players get @s MissionTime
execute if entity @s[tag=add_bossbar] store result bossbar medabots_server:ruins_in_b/robattle value run scoreboard players get @s MissionTime
execute if entity @s[tag=add_bossbar] run scoreboard players reset @s MissionTime
tag @s[tag=add_bossbar] remove add_bossbar
execute as @e[x=0,y=9,z=0,dx=92,dy=49,dz=92,tag=build_stage,type=minecraft:item,limit=1] run data modify entity @s Item.tag.display.Lore[0] set value '{"italic":false,"color":"white","translate":"medabots_server:item.custom_stage.location","with":[{"translate":"medabots_server:location.ruins_in_b"}]}'
...
execute if entity @s[tag=clear_stage] positioned 46 59 46 run function medabots_server:stage/clear