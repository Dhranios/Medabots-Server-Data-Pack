...
execute unless entity @a[scores={State=0..2,Stage=3,StageVersion=2}] run setblock -469 92 268 minecraft:sea_lantern
execute unless entity @a[scores={State=0..2,Stage=3,StageVersion=2}] run data merge block -468 91 268 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function medabots_server:stage/sign/hard_fly_course_1/initiate_race"}}',Text3:'{"translate":"medabots_server:sign.stage.fly_course.race"}'}
execute unless entity @a[scores={State=0..2,Stage=3,StageVersion=2}] unless entity @a[scores={State=3,Stage=3,StageVersion=2},tag=calculate_results] run tag @s add calculate_results