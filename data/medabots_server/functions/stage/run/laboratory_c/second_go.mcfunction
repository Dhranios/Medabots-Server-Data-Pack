...
execute if block -1586 44 -598 minecraft:iron_trapdoor[open=true] if entity @s[x=-1587,y=44,z=-601,dx=3,dy=3,dz=3] run function medabots_server:stage/create/laboratory_c/second_go_battle/0
execute if entity @e[x=-1596,y=42,z=-620,dx=22,dy=7,dz=44,tag=mr_referee,type=minecraft:armor_stand,tag=finished] store result score @s BattlingMedabots if entity @e[scores={Stage=28,Medabot=0..,State=1..2}]
...
advancement grant @s[scores={BattlingMedabots=1,State=1..}] only medabots_server:stages/wave_1/laboratory_c_second_go
...
execute if entity @s[scores={BattlingMedabots=1,State=1..}] positioned -1586 50 -599 run function medabots_server:stage/clear
