...
execute if entity @e[scores={Stage=30},tag=hide_normal_time] if score #temp Time matches 0 as @e[x=-1600,y=42,z=-750,dx=62,dy=7,dz=62,tag=mission,type=minecraft:area_effect_cloud] at @s run function medabots_server:stage/mission_time_up
...
execute if entity @s[scores={Battle=0},tag=!stage_builder] run function medabots_server:stage/clean_up/grassland_d
...
execute as @e[x=-1600,y=42,z=-750,dx=62,dy=7,dz=62,tag=build_stage,type=minecraft:item,limit=1] run data modify entity @s Item.tag.display.Lore[0] set value '{"italic":false,"color":"white","translate":"medabots_server:item.custom_stage.location","with":[{"translate":"medabots_server:location.grassland_d"}]}'
