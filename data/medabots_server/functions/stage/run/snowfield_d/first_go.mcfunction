...
execute if block -1563 45 -796 minecraft:iron_trapdoor[open=true] if entity @s[x=-1563,y=45,z=-797,dx=3,dy=3,dz=3] run function medabots_server:stage/create/snowfield_d/first_go_battle/0
execute if entity @e[x=-1602,y=42,z=-815,dx=61,dy=7,dz=62,tag=mr_referee,type=minecraft:armor_stand,tag=finished] store result score @s BattlingMedabots if entity @e[scores={Stage=32,Medabot=0..,State=1..2}]
execute as @e[x=-1602,y=42,z=-815,dx=61,dy=7,dz=62,type=!minecraft:player] unless entity @s[scores={Stage=0..}] run scoreboard players set @s Stage 32
...
execute if entity @s[scores={BattlingMedabots=1,State=1..}] positioned -1572 51 -784 run function medabots_server:stage/clear
