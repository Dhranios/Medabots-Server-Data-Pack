...
execute if entity @s[scores={Battle=0},tag=!stage_builder] run function medabots_server:stage/clean_up/ruins_out_c
...
execute if entity @s[tag=add_bossbar] run bossbar set medabots_server:ruins_out_c/robattle value 3600
tag @s[tag=add_bossbar] remove add_bossbar
execute as @e[x=-1665,y=0,z=-704,dx=62,dy=49,dz=63,tag=build_stage,type=minecraft:item,limit=1] run data modify entity @s Item.tag.display.Lore[0] set value '{"italic":false,"color":"white","translate":"medabots_server:item.custom_stage.location","with":[{"translate":"medabots_server:location.ruins_out_c"}]}'
