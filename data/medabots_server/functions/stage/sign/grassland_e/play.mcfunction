...
teleport @s[scores={Dialog=0},gamemode=adventure] 46 45 51 90 0
execute if entity @s[scores={Dialog=0},gamemode=adventure] run data merge block 287 60 -503 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function medabots_server:stage/try/grassland_e/enter"}}'}
execute if entity @s[scores={Dialog=0},gamemode=adventure] run setblock 287 61 -505 minecraft:air
...
scoreboard players set @s[scores={Dialog=0},gamemode=adventure] Stage 52
