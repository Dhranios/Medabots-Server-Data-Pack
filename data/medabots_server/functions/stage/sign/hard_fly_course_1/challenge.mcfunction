...
execute unless entity @s[scores={StageIndex=-1..}] run scoreboard players set @s[scores={Dialog=0,Verified=3},gamemode=adventure] Stage 3
execute unless entity @s[scores={StageIndex=-1..}] if entity @s[scores={Dialog=0,Verified=3},gamemode=adventure] run function medabots_server:stage/fly_course/next_challenge_ring/hard_1
execute unless entity @s[scores={StageIndex=-1..}] run teleport @s[scores={Dialog=0,Verified=3},gamemode=adventure] -510.0 89 263.0 0 0
