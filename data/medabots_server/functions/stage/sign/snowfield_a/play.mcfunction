...
teleport @s[scores={Dialog=0},gamemode=adventure] 64 45 24 90 0
execute if entity @s[scores={Dialog=0},gamemode=adventure] run data merge block 47 60 0 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function medabots_server:stage/try/snowfield_a/enter"}}'}
execute if entity @s[scores={Dialog=0},gamemode=adventure] run setblock 47 61 -2 minecraft:air
...
scoreboard players set @s[scores={Dialog=0},gamemode=adventure] Stage 5
