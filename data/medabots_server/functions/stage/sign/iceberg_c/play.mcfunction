...
teleport @s[scores={Dialog=0},gamemode=adventure] 28 38 33 90 0
execute if entity @s[scores={Dialog=0},gamemode=adventure] run data merge block 28 60 1 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function medabots_server:stage/try/iceberg_c/enter"}}'}
execute if entity @s[scores={Dialog=0},gamemode=adventure] run setblock 28 61 -1 minecraft:air
...
scoreboard players set @s[scores={Dialog=0},gamemode=adventure] Stage 26
