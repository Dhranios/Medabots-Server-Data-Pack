...
teleport @s[scores={Dialog=0},gamemode=adventure] 38 38 32 90 0
execute if entity @s[scores={Dialog=0},gamemode=adventure] run data merge block 30 60 0 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function medabots_server:stage/try/grassland_c/enter"}}'}
execute if entity @s[scores={Dialog=0},gamemode=adventure] run setblock 30 61 -2 minecraft:air
...
scoreboard players set @s[scores={Dialog=0},gamemode=adventure] Stage 23
