...
execute if entity @s[scores={StageIndex=-1}] run tellraw @a {"translate":"medabots_server:message.stage.fly_course.canceled","color":"green","with":[{"translate":"medabots_server:location.hard_fly_course_1"}]}
...
execute if entity @s[scores={StageIndex=-1}] run setblock -469 92 266 minecraft:stone_button[face=wall,facing=north,powered=true]
