...
execute if entity @a[scores={Stage=62,Gamemode=0,StageIndex=-1},limit=1] if entity @s[gamemode=adventure,tag=valid_tinpet,scores={Dialog=0},advancements={medabots_server:special_items/passes/smash_melee=true}] run function medabots_server:stage/join/final_destination_2/join
...
execute if entity @s[advancements={medabots_server:special_items/passes/smash_melee=false},tag=valid_tinpet] run tellraw @s[gamemode=adventure] {"translate":"medabots_server:message.stage.requires.not_matching","with":[{"translate":"medabots_server:message.stage.requirements.final_destination_2.0"}]}
