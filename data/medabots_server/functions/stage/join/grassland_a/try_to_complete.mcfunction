...
scoreboard players operation #temp NeededPlayers -= @a[distance=..1,scores={StageIndex=0..},limit=1] StageIndex
execute if entity @a[distance=..1,scores={StageIndex=0..},limit=1] run scoreboard players remove #temp NeededPlayers 1

execute if score #temp NeededPlayers matches 0 run tellraw @a {"translate":"medabots_server:message.stage.players","color":"green","with":[{"translate":"medabots_server:location.grassland_a"}]}
...
execute if score #temp NeededPlayers matches 0 run data merge block 30 60 0 {Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function medabots_server:stage/try/grassland_a/enter"}}',Text3:'{"translate":"medabots_server:sign.stage.play.main_player"}'}
