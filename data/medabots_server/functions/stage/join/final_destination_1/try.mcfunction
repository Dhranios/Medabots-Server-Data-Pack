...
execute if entity @s[advancements={medabots_server:special_items/passes/smash_bros=false}] run tellraw @s[gamemode=adventure] {"translate":"medabots_server:message.stage.requires.not_matching","with":[{"translate":"medabots_server:message.stage.requirements.final_destination_1.0"}]}
