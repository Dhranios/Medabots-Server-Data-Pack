...
execute if entity @a[scores={Stage=61,Gamemode=0,StageIndex=-1},limit=1] if entity @s[gamemode=adventure,tag=valid_tinpet,scores={Dialog=0},advancements={medabots_server:special_items/passes/smash_bros=true}] run function medabots_server:stage/join/final_destination_1/join
...
execute if entity @s[advancements={medabots_server:special_items/passes/smash_bros=false}] run tellraw @s[gamemode=adventure,tag=valid_tinpet] {"translate":"medabots_server:message.stage.requires.not_matching","with":[{"translate":"medabots_server:message.stage.requirements.final_destination_1.0"}]}
