...
execute at @s if entity @a[scores={Stage=2,Gamemode=1,StageIndex=-1},limit=1,distance=0.1..] if entity @s[gamemode=adventure,scores={Dialog=0}] unless score @s Money >= @a[scores={Stage=2,Gamemode=1,StageIndex=-1},limit=1] FlyCourseFee run tellraw @s {"translate":"medabots_server:message.stage.fly_course.no_money","color":"green"}

execute at @s if entity @a[scores={Stage=2,Gamemode=1,StageIndex=-1},limit=1,distance=0.1..] if entity @s[gamemode=adventure,scores={Dialog=0}] if score @s Money >= @a[scores={Stage=2,Gamemode=1,StageIndex=-1},limit=1] FlyCourseFee run function medabots_server:stage/join/medium_fly_course_1/join
