...
execute if entity @s[scores={Moving=1..}] positioned ~ ~1 ~ as @e[distance=..0.6,tag=cannon,type=minecraft:husk] at @s run fill ~ ~2 ~ ~ ~2 ~ minecraft:air replace minecraft:barrier
execute if entity @s[scores={Moving=1..}] positioned ~ ~1 ~ as @e[distance=..0.6,tag=cannon,type=minecraft:husk] at @s run fill ~ ~ ~ ~ ~ ~ minecraft:air replace minecraft:iron_block
