...
execute at @s[scores={Time=45}] as @e[tag=master_hand] if score @s Stage = #temp Stage run particle minecraft:explosion ~-0.25 ~0.25 ~-0.25 0.5 0.5 0.5 0 10
execute if entity @s[scores={Time=45}] as @e[tag=master_hand] if score @s Stage = #temp Stage run playsound medabots_server:entity.crazy_hand.power_punch hostile @a ~ ~ ~ 1

execute if entity @s[scores={Time=46..50}] run teleport @s ^ ^0.1 ^-1.2
execute if entity @s[scores={Time=86..90}] run teleport @s ^ ^-0.1 ^
execute if entity @s[scores={Time=65}] run playsound medabots_server:entity.crazy_hand.hard_hit hostile @a ~ ~ ~ 1
execute if entity @s[scores={Time=82}] run playsound medabots_server:entity.crazy_hand.hard_hit hostile @a ~ ~ ~ 1
