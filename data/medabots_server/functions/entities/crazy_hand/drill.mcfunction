...
execute if entity @s[scores={Time=1}] run playsound medabots_server:entity.crazy_hand.spin_start hostile @a ~ ~ ~ 1
...
teleport @s[scores={Time=20}] ~ ~-1 ~
execute if entity @s[scores={Time=11}] run playsound medabots_server:entity.crazy_hand.spin_land hostile @a ~ ~-40 ~ 1
execute if entity @s[scores={Time=19}] run playsound medabots_server:entity.crazy_hand.spin_stop hostile @a ~ ~ ~ 1

execute if entity @s[scores={Time=1..60}] run effect give @e[tag=potential_target,distance=..3] minecraft:instant_damage 1 0 true
execute if entity @s[scores={Time=60}] unless block ~ ~-3 ~ minecraft:air run playsound medabots_server:entity.crazy_hand.spin_return hostile @a ~ ~ ~ 1
