...
execute unless entity @s[scores={Stage=1..}] if entity @e[type=minecraft:villager,distance=..4] run tellraw @s[scores={Dialog=0,Money=..99}] {"translate":"chat.type.text","with":[{"translate":"medabots_server:entity.salesman"},{"translate":"medabots_server:message.shop.too_expensive"}]}
