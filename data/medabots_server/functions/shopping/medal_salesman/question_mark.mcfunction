tag @s add success
execute if entity @s[advancements={medabots_server:get_all_medabots/wave_1/question_medal=false}] run tag @s add out_of_stock
...
tellraw @s[advancements={medabots_server:get_all_medabots/wave_1/question_medal=true}] {"translate":"medabots_server:message.shop.buy","clickEvent":{"action":"run_command","value":"/trigger Shopping set 15"},"with":[{"translate":"medabots_server:item.question_medal"},{"text":"500"}]}
