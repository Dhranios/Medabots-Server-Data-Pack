...
execute unless entity @s[advancements={medabots_server:get_all_medabots/wave_1/galleom={legs=false,left_arm=false,right_arm=false,head=false}}] run tellraw @s {"clickEvent":{"action":"run_command","value":"/trigger ShopIndex4 set 15"},"translate":"medabots_server:advancements.get_all_medabots.galleom"}}
execute unless entity @s[advancements={medabots_server:get_all_medabots/wave_1/master_core={legs=false,left_arm=false,right_arm=false,head=false}}] run tellraw @s {"clickEvent":{"action":"run_command","value":"/trigger ShopIndex4 set 16"},"translate":"medabots_server:advancements.get_all_medabots.master_core"}}
