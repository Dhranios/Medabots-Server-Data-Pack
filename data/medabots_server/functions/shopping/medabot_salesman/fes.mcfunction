...
execute unless entity @s[advancements={medabots_server:get_all_medabots/wave_1/nino={legs=false,left_arm=false,right_arm=false,head=false}}] run tellraw @s {"clickEvent":{"action":"run_command","value":"/trigger ShopIndex4 set 14"},"translate":"medabots_server:advancements.get_all_medabots.nino"}}
execute unless entity @s[advancements={medabots_server:get_all_medabots/wave_1/amelia={legs=false,left_arm=false,right_arm=false,head=false}}] run tellraw @s {"clickEvent":{"action":"run_command","value":"/trigger ShopIndex4 set 13"},"translate":"medabots_server:advancements.get_all_medabots.amelia"}}
