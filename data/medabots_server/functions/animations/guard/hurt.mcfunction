execute if entity @e[type=minecraft:creeper,tag=this_guard,limit=1,tag=attack] run scoreboard players set @s AnimationProg 0
...
execute if entity @s[scores={AnimationProg=1}] as @e[type=minecraft:creeper,tag=this_guard,limit=1] run function medabots_server:entities/guard/hurt
data merge entity @s[scores={AnimationProg=2}] {Pose:{Head:[0.001f,0.001f,0.001f]}}
scoreboard players reset #temp Time
execute if entity @s[scores={AnimationProg=2}] run tag @e[type=minecraft:creeper,tag=this_guard,limit=1] remove hurt
