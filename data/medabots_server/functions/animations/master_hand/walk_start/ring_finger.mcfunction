...
execute if entity @s[scores={AnimationProg=10..19}] at @e[type=minecraft:ghast,tag=this_master_hand,limit=1] rotated ~ 0 run teleport @s ^ ^-0.1 ^0.1
execute if entity @s[scores={AnimationProg=20}] at @e[type=minecraft:ghast,tag=this_master_hand,limit=1] rotated ~ 0 run teleport @s ^ ^-0.5 ^0.6