...
execute if entity @s[scores={AnimationProg=9..13}] store result entity @s Pose.Head[0] float 1 run scoreboard players remove #temp Time 17
scoreboard players reset #temp Time
execute if entity @s[scores={AnimationProg=1..8}] at @e[type=minecraft:ghast,tag=this_master_hand,limit=1] rotated ~ 0 run teleport @s ^1.3 ^0.6 ^
