...
execute unless entity @s[tag=!walking,tag=!running,tag=!sneaking] if entity @s[nbt=!{ArmorItems:[{tag:{medabots_server:{move:"multi_legged"}}}]},nbt=!{ArmorItems:[{tag:{medabots_server:{leg_data:{leg_count:0}}}}]},tag=!dancing,tag=!carried] run function medabots_server:animations/medabot/walking/two_legged_legs
