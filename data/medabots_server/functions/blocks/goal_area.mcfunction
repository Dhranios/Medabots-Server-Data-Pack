...
execute as @e[tag=mission,type=minecraft:area_effect_cloud] if score @s Stage = #temp Stage run tag @e[distance=..0.7,tag=goal_area] add stop
scoreboard players reset #temp Stage
execute if entity @s[tag=!stop] align xz run tag @a[dx=0,dy=4,dz=0,tag=hostile] add clear_stage
tag @s remove stop
