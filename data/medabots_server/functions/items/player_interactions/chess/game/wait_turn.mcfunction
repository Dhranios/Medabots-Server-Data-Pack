...
execute if entity @s[tag=did_move] if entity @e[type=minecraft:armor_stand,tag=origin,limit=1,tag=turn_white,tag=this_chess_game] run tellraw @a[tag=this_chess_game] {"translate":"medabots_server:message.player_interactions.chess.turn.black","with":[{"selector":"@a[tag=this_chess_game,tag=awaiting_chess_response]"}],"color":"green"}
