...
scoreboard players operation #temp3 Money /= #10 Constants
scoreboard players operation #temp2 Money /= #100 Constants
in minecraft:overworld run setblock -286 3 -52 minecraft:oak_sign{Text1:'{"italic":false,"color":"white","translate":"medabots_server:item.loot_container.medallars","with":[{"translate":"medabots_server:item.loot_container.medallars.value","with":[{"score":{"objective":"Money","name":"#temp2"}},{"score":{"objective":"Money","name":"#temp3"}},{"score":{"objective":"Money","name":"#temp"}}]}]}'}
in minecraft:overworld run data modify entity @e[tag=inventory,limit=1] ArmorItems[3].tag.medabots_server.items[{tag:{medabots_server:{id:"medabots_server:loot_container"}}}].tag.display.Lore[2] set from block -286 3 -52 Text1
