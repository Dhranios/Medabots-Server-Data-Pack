execute if entity @s[scores={Time=20}] run function medabots_server:items/medapart/beam/20
execute if entity @s[scores={Time=60..}] run function medabots_server:items/medapart/beam/60_

# Increase time by 1
scoreboard players add @s[scores={MedapartType=40}] Time 1
