...
execute unless entity @s[scores={SelfDestructTime=1..}] run scoreboard players add @s[scores={SelfDestruct=1..,State=1},nbt={SelectedItem:{tag:{medabots_server:{part:"medal"}}}}] SelfDestructTime 1
execute unless entity @s[scores={SelfDestructTime=1..}] run scoreboard players add @s[scores={SelfDestruct=1..,State=1},type=!minecraft:player] SelfDestructTime 1
